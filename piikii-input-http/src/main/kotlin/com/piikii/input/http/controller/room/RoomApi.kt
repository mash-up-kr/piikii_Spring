...
class RoomApi(
    ...
    @PostMapping
    fun saveRoom(@RequestBody request: RoomSaveRequestForm): ResponseEntity<ResponseForm<RoomSaveResponseForm>> {
        val room = roomUseCase.save(request.toDomain())
        return ResponseEntity.created(URI("/")).body(ResponseForm(
    ...
    @PutMapping("/{roomId}")
    fun updateRoom(@RequestBody request: RoomUpdateRequestForm, @PathVariable roomId: Long): ResponseEntity<ResponseForm<Any>> {
    ...
    fun deleteRoom(@PathVariable roomId: Long): ResponseEntity<ResponseForm<Any>> {
    ...
    @GetMapping("/{roomId}")
    fun getRoom(@PathVariable roomId: Long): ResponseEntity<ResponseForm<RoomGetResponseForm>> {
